...
local function uint_to_hex(num, len, be)
    ...
    local out = ''
    for i = 1, #raw do
        out = out .. string_format("%02x", raw:byte(i,i))
    end
    return out
end
...
local function generate_oid()
    ...
    counter = counter + math_random(1,200)
    counter = counter < 2^24 and counter or 1  -- out of rage,set to 1
    return uint_to_hex(ngx_now(), 4, 1) .. machineid .. pid .. uint_to_hex(counter, 4, 1):sub(3,8)
end
...
local function xor_bytestr( a, b )
    local res = ""    
    for i=1,#a do
        res = res .. string_char(bit.bxor(string_byte(a,i,i), string_byte(b, i, i)))
    end
    return res
end
...
local function saslprep(username)
    return string_gsub(string_gsub(username, '=', '=3D'), ',' , '=2C')
end
